...
5. 分阶段执行核心数据库迁移、写入系统初始数据、原子生成 `.env`、写入安装标记，并返回实时阶段进度。
...
- 首次安装和后续升级共用 `database/migrations/` 中的 Phinx 核心迁移；基础迁移能够接管当前旧安装器已创建的表，系统初始数据使用幂等写入。
...
数据库结构和系统版本的后续更新流程见 [数据库迁移与系统版本更新规范](database-migrations.md)。
